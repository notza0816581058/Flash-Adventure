...
public class Game extends JPanel implements KeyListener{
	...
	private static int speed = 50,flashSize = 60 ,waveHeight = 50;
	private static int base=400,xStart = 1000;
	private long point = 0,lastPress=0;
	
	private Flash flash = new Flash(100,base-50);
...
//--------------------Cloud--------------------------------
	...
	public void paint(Graphics g) {
			try {
				...
				drawFlashHealth(g2);
				g2.drawImage(flash.getImage(),flash.x,flash.y,flashSize,flashSize, null);
				//----Wave----
	...
	private void drawBackground(Graphics2D g2) throws IOException {
			g2.drawImage(ImageIO.read(new File("img\\sky.jpg")),0,0,2000,1000, null);
	...
	private void drawFlashHealth(Graphics2D g2) {
		try {
			...
			g2.drawLine(60, 30,60+flash.health,30);	
			g2.setColor(Color.white);
	...
	private void drawWave(Wave wave,Graphics2D g2) {
			...
			if(Event.checkHit(flash,wave,flashSize,waveHeight)){
					g2.setColor(new Color(241, 98, 69));
					g2.fillRect(0, 0,1000,1000);			
					flash.health-=20;
					if(flash.health<=0) {
						display.endGame(this.point);
						flash.health = new Flash().health;
						this.point = 0;	
					}
			}
	}
